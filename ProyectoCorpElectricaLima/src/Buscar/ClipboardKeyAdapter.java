...
public class ClipboardKeyAdapter extends KeyAdapter {
        ...
        public void keyReleased1(KeyEvent event1){
        	if(event1.isControlDown()){
        		if(event1.getKeyCode()==KeyEvent.VK_C){
        			cancelEditing();
        			copyToClipboard(false);
        			
        		}else if (event1.getKeyCode()==KeyEvent.VK_X) {
        			cancelEditing();
        			copyToClipboard(true);
					
				}else if (event1.getKeyCode()==KeyEvent.VK_V) {
					cancelEditing();
					pasteFromClipboard();
        ...
        private void copyToClipboard(boolean isCut) { 
                ...
                int numcols1=table.getSelectedColumnCount();
                int numrows1=table.getSelectedColumnCount();
                int numrows2=table.getSelectedColumnCount();
