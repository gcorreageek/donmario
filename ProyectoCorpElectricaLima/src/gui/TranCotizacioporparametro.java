 ...
 */
public class TranCotizacioporparametro extends JInternalFrame implements
	...
	String COD_PROD, NOM_PROD, COSTE, COD_MAR, MARCA, COD_UMED, UMED, FECHA,
			COD_PROVEE, PROVEEDOR, COD_PRODALT, NOM_PRODALT, COD_DET_DIF,
			PESO_PRODUCTO;
	...
	String ref = "";
	String titulo2[] = { "Id", "Cant", "Und", "Descripcion", "Marca",
			"P.Unit$", "P.Total$", "P.Costo$", "C.Total$", "Dif.$", "Fecha",
			"%", "Proveedor", "Peso Producto", "T.Entrega" };
	...
	public TranCotizacioporparametro() {
		...
		try {
			...
			pnlPricncipal.setBackground(Color.LIGHT_GRAY);
			pnlPricncipal.setForeground(Color.LIGHT_GRAY);
			...
			pnlAbajo = new JPanel();
			pnlAbajo.setBackground(Color.DARK_GRAY);
			...
			btnNewButton.setBounds(10, 1, 89, 23);
			pnlAbajo.add(btnNewButton);
			...
			// String
			// titulo2[]={"Cant","Und","Descripcion","Marca","P.Total $","Total $","Costo $","Total $","Dif. $","Fecha","%","Proveedor"};
			// String
			// titulo2[]={"Id","Cant","Und","Descripcion","Marca","P.Unit$","P.Total$","P.Costo$","C.Total$","Dif.$","Fecha","%","Proveedor"};
			TableColumn Cant = tblLista.getColumn("Cant"), Und = tblLista
					.getColumn("Und"), Descripcion = tblLista
					.getColumn("Descripcion"), Marca = tblLista
					.getColumn("Marca"), PTotal = tblLista.getColumn("P.Unit$"), TotalD = tblLista
					.getColumn("P.Total$"), Costo = tblLista
					.getColumn("P.Costo$"), Total = tblLista
					.getColumn("C.Total$"), Dif = tblLista.getColumn("Dif.$"), Fecha = tblLista
					.getColumn("Fecha"), por = tblLista.getColumn("%"), Proveedor = tblLista
					.getColumn("Proveedor"), iD = tblLista.getColumn("Id"), Peso = tblLista
					.getColumn("Peso Producto");
			// Id
	...
	TableCellRenderer renderer = new CustomTableCellRendererAcercamiento1();
public void  PintarFila(){
		...
		TableColumn column14 = tblLista.getColumn(tblLista.getColumnName(14));		
	...
	public double pasarPorceGana(int cod_prod) {
		/*
		 * SELECT RUB.POR_RUBRO FROM PRODUCTO PROD INNER JOIN RUBRO RUB ON
		 * PROD.COD_RUBRO=RUB.COD_RUBRO WHERE PROD.COD_PROD='1';
		 */
	...
	public void calcularParte1(int i) {
		...
		modelo2.setValueAt(Double.parseDouble("" + formato(precioConPorce)), i,
				5);// P.Total $
		modelo2.setValueAt(
				Double.parseDouble("" + formato(precioConPorceXCant)), i, 6);// Total
																				// $
		modelo2.setValueAt(Double.parseDouble("" + formato(costoreal)), i, 7);// Costo
																				// $
		modelo2.setValueAt(Double.parseDouble("" + formato(costoRealXCant)), i,
				8);// Total $
	...
	public String sinAlgunasPalabrasYSimbolos(String cad) {
		...
		/*
		 * cad13=cad12.replaceAll("long", ""); cad14=cad13.replaceAll(".", "");
		 * cad15=cad14.replaceAll("c/", ""); cad16=cad15.replaceAll("p/", "");
		 */
	...
	public String encontrarProducto(String cad) {
		...
		String sql = "SELECT * FROM tb_producto "
				+ "where NOM_PROD like"
				+ " (select(lcase(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace('"
				+ cad
				+ "','long',''),"
	...
	public String encontrarModelo(String cad) {
		...
		String sql = "SELECT * FROM tb_producto " + "where MOD_PROD='" + cad
				+ "' " + " and EST_PROD='ACTIVADO' limit 1;";
		// System.out.println(sql);
		ResultSet rs = objAccesoBD.ejecutarConsulta(sql);
		String nom = "";
	...
	public String encontrarCodPromelsa(String cad) {
		...
		String sql = "SELECT * FROM tb_producto " + "where CODPRO_PROD='" + cad
				+ "' " + "and EST_PROD='ACTIVADO' limit 1;";
		// System.out.println(sql);
		ResultSet rs = objAccesoBD.ejecutarConsulta(sql);
		String nom = "";
	...
	public String existeProd(String cad) {
		...
		String sql = "SELECT * FROM tb_producto " + "where NOM_PROD='" + cad
				+ "' " + " and EST_PROD='ACTIVADO' limit 1;";
		// System.out.println(sql);
		ResultSet rs = objAccesoBD.ejecutarConsulta(sql);
		String nom = "";
	...
	public void reescribir() {
		...
		for (int i = 0; i < modelo2.getRowCount(); i++) {
			...
			String uMed = "" + modelo2.getValueAt(i, 2), modelo = objExcel
					.getModelo().trim(), codprom = objExcel.getCodPromelsa()
					.trim();

			nom0 = primeraSinS(primerapal(productoViene.trim()))
					+ sinAlgunasPalabrasYSimbolos((completarOracion(productoViene
							.trim())));
			...
			if (nom3.equals("")) {// MODELO
				...
				if (nom3.equals("")) {// COD. PROMELSA
					if (productoViene.equals("")
							|| productoViene.equals("null")) {
						nom3 = "";
					} else {
						if (nom0.equals("") || nom0.equals("0.0")
								|| nom0.equals("null")) {
							nom3 = "";
						} else {
							nom1 = "%" + nom0 + "%";
	...
	public void buscarDetalles() {
		...
		for (int i = 0; i < modelo2.getRowCount(); i++) {
			...
			String productoViene = "" + modelo2.getValueAt(i, 3);
			// String
			// nom="%"+primeraSinS(primerapal(productoViene))+completarOracion(productoViene)+"%";
			...
			if (vamosAver.equals("0")) {
				...
				int viene = objMetodo.buscar1parte1(productoViene.trim(),
						umedViene.trim());// PERATE
				if (viene == 0) {
					...
					codprod = objMetodo.codprod;
					codmar = objMetodo.codmar;
					codprove = objMetodo.codprove;
					cod_umed = objMetodo.cod_umed;
					producto = objMetodo.producto;
					mon = objMetodo.mon;
					umed = objMetodo.umed;
					obsprod = objMetodo.obsprod;
					obsdet = objMetodo.obsdet;
					igv = objMetodo.igv;
					marcar = objMetodo.marca;
					proveedor = objMetodo.proveedor;
					fec = objMetodo.fec;
					cod_prodalt = objMetodo.cod_prodalt;
					nom_prodalt = objMetodo.nom_prodalt;
					costoreal = objMetodo.costoreal;
					costoprove = objMetodo.costoprove;
					peso = objMetodo.peso;
					...
					if (umedViene.equals(umed)) {
						...
						if (vamosAver.equals("1")) {
							...
							double precioConPorce = costoreal / porc;
							precioConPorce = formato(precioConPorce);
							double precioConPorceXCant = precioConPorce
									* cantViene;
							precioConPorceXCant = formato(precioConPorceXCant);
							double dife = precioConPorceXCant - costoRealXCant;
							double pesoTotal = peso;
			...
			// SI ES MAYOR QUE 2
				...
				// ESTE ELSE SIGNIFICA QUE ENCONTRO EL PRODUCTO PERO HAY MAS DE
				// UN PROVEEDOR
				// QUE VENDE ESTE PRODUCTO POR ESO QUE SALEN VARIOS CON
				// DIFERENTES PRECIOS
				int viene = objMetodo.buscar2parte1(productoViene.trim(),
						umedViene.trim());
				if (viene == 0) {
					...
					// ESTO SI ES UN GRAN ERROR ,QUE NO CREO QUE SALGA NUNCA
										 ...
										 * ESTE BUSCA SI HAY UN PRODUCTO CON EL
										 * MISMO PRECIO(MINIMO)
										 */
					...
					int viene2 = objMetodo.buscar2parte1(productoViene.trim(),
							"mayyyyyyyyyy");
					if (viene2 == 0) {
						...
						// CHEVRE PORQUE YA ENCONTRO EL PRODUCTO INDICADO POR LA
						// MARCA
						// chevre encontro e imprime
						codprod = objMetodo.codprod;
						codmar = objMetodo.codmar;
						...
						// MAL PORQUE SIGEN HABIENDO 2 O MAS PRODUCTOS CON EL
						// MISMO PRECIO Y CON LA MISMA MARCA
			...
			/*
			 * int codprod,codmar,codprove; String
			 * producto,mon,umed,obsprod,obsdet,igv,marcar,proveedor,fec; double
			 * costoreal,costoprove;
			 * 
			 * int codprod,codmar,codprove,cod_umed; String
			 * producto,mon,umed,obsprod,obsdet,igv,marca,proveedor,nom_prodalt;
			 * double costoreal,costoprove; String fec,cod_det_dif;
			 */
			BeanVarios objVar = new BeanVarios(codprod, codmar, codprove,
					cod_umed, producto, mon, umed, obsprod, obsdet, igv,
					marcar, proveedor, nom_prodalt, costoreal, costoprove, fec,
					cod_det_dif);
			 ...
			 * ************************************
			 */
			BeanCoti_borrar objcotborrar = new BeanCoti_borrar(
					("" + modelo2.getValueAt(i, 1)), ("" + modelo2.getValueAt(
							i, 2)), ("" + modelo2.getValueAt(i, 3)),
					("" + modelo2.getValueAt(i, 4)), ("" + modelo2.getValueAt(
							i, 5)), ("" + modelo2.getValueAt(i, 6)),
					("" + modelo2.getValueAt(i, 7)), ("" + modelo2.getValueAt(
							i, 8)), ("" + modelo2.getValueAt(i, 9)),
					("" + modelo2.getValueAt(i, 10)), ("" + modelo2.getValueAt(
							i, 11)), ("" + modelo2.getValueAt(i, 12)),
					("" + modelo2.getValueAt(i, 13)), ("" + modelo2.getValueAt(
							i, 14)));
			objCotizacionBorrar.adicionar(objcotborrar);
			// AQUI TIENES QUE PONER EL BEANS DONDE SE VA AGUARDAR UNA VES YA
			// PUESTO,
	...
	private boolean abrirArchivo() {
		...
		//	return false;"D://WK//ProyectoCorpElectricaLima//EXPORTAR.xls"
		...
		if (tam > 0) {
			for (int i = 0; i < arrayExcel.tamaño(); i++) {
				...
				Object obj[] = { objExcel.getId(), objExcel.getCant(),
						objExcel.getUmedida(), objExcel.getDescripcion(),
						objExcel.getMarca() };
				modelo2.addRow(obj);
			}
		}
		/*
		 * String
		 * CODCLI,NOMCLI,TIPCLI,RUCCLI,LUGCLI,CONACLI,TEL1ACLI,TEL2ACLI,RPMACLI
		 * ,NEXACLI,CELACLI,MAILCLI;
		 */
	...
	public void cargarCotizacionInc() {
		...
		String sql = "select ide_cot_inc,num_item,cod_ven,cod_cli,fecha_coti,cantidad,unimedida,desc_prod,marca,precio_unit,precio_total, "
				...
				+ fecha_coti
				+ "' and COD_VEN='"
				+ cod_ven + "' and IDE_COT_INC='" + ide_cot_inc + "'; ";
		...
		try {
			...
			while (rs.next()) {
				// conta_coti++;
				BeanCotizacionInconclusa objR = new BeanCotizacionInconclusa(
						rs.getString(1), rs.getString(2), rs.getString(3),
						rs.getString(4), rs.getString(5), Integer.parseInt(rs
								.getString(6)), rs.getString(7),
						rs.getString(8), rs.getString(9), rs.getString(10),
						rs.getString(11), rs.getString(12), rs.getString(13),
						rs.getString(14), rs.getString(15), rs.getString(16),
						rs.getString(17), rs.getInt(18), rs.getInt(19),
						rs.getInt(20), rs.getInt(21), rs.getString(22),
						rs.getString(23), rs.getDouble(24));
		...
		for (int i = 0; i < objSCotInc.tamaño(); i++) {
			...
			Object[] obj = { objSCotInc.obtener(i).getNum_item(),
					objSCotInc.obtener(i).getCant(),
					objSCotInc.obtener(i).getUnimedida(),
					objSCotInc.obtener(i).getDesc_prod(),
					objSCotInc.obtener(i).getMarca(),
					objSCotInc.obtener(i).getPrecio_unit(),
					objSCotInc.obtener(i).getPrecio_total(),
					objSCotInc.obtener(i).getPrecio_costo(),
					objSCotInc.obtener(i).getCosto_total(),
					objSCotInc.obtener(i).getDiferencia(),
					objSCotInc.obtener(i).getFecha(),
					objSCotInc.obtener(i).getPorcentaje(),
					objSCotInc.obtener(i).getNom_prove(),
					objSCotInc.obtener(i).getPeso_prod(),
					objSCotInc.obtener(i).getTiempo_entrega() };
			modelo2.addRow(obj);
			/******************************************************************************************/
			cant = "" + objSCotInc.obtener(i).getCant();
			BeanCoti_borrar objcotborrar = new BeanCoti_borrar(cant, objSCotInc
					.obtener(i).getUnimedida(), objSCotInc.obtener(i)
					.getDesc_prod(), objSCotInc.obtener(i).getMarca(),
					objSCotInc.obtener(i).getPrecio_unit(), objSCotInc.obtener(
							i).getPrecio_total(), objSCotInc.obtener(i)
							.getPrecio_costo(), objSCotInc.obtener(i)
							.getCosto_total(), objSCotInc.obtener(i)
							.getDiferencia(), objSCotInc.obtener(i).getFecha(),
					objSCotInc.obtener(i).getPorcentaje(), objSCotInc
							.obtener(i).getNom_prove(), objSCotInc.obtener(i)
							.getPeso_prod(), objSCotInc.obtener(i)
							.getTiempo_entrega());
	...
	public void cargarCliCoti() {
		...
		String sql = "SELECT CLI.NOM_CLI,CLI.CONA_CLI,CLI.DIR_CLI,CLI.TEL1A_CLI,CLI.RPMA_CLI,CLI.NEXA_CLI,CLI.CELA_CLI,CLI.MAILA_CLI,TIPCLI.nom_tipo "
				...
				+ "WHERE COTINC.FECHA_COTI='"
				+ fecha_coti
				+ "' AND COTINC.COD_VEN='"
				+ cod_ven
				+ "' AND COTINC.IDE_COT_INC='" + ide_cot_inc + "';";
		...
		try {
			...
			while (rs.next()) {
				// conta_coti++;
				BeanCliCoti objR = new BeanCliCoti(rs.getString(1),
		 ...
		 * 
		 * Object[]
		 * obj={objSCotInc.obtener(i).getNum_item(),objSCotInc.obtener(i
		 * ).getCant(),
		 * objSCotInc.obtener(i).getUnimedida(),objSCotInc.obtener(i
		 * ).getDesc_prod(),
		 * objSCotInc.obtener(i).getMarca(),objSCotInc.obtener(
		 * i).getPrecio_unit(),
		 * objSCotInc.obtener(i).getPrecio_total(),objSCotInc
		 * .obtener(i).getPrecio_costo(),
		 * objSCotInc.obtener(i).getCosto_total(),
		 * objSCotInc.obtener(i).getDiferencia(),
		 * objSCotInc.obtener(i).getFecha(
		 * ),objSCotInc.obtener(i).getPorcentaje(),
		 * objSCotInc.obtener(i).getNom_prove() }; modelo2.addRow(obj);
		 * 
		 * }
		 */
	}
	...
	public void calcular() {
		...
		double precioTsuma = 0, costoTsuma = 0, difTsuma = 0, precioTsumaSigv = 0, costoTsumaSigv = 0, difTsumaSigv = 0, precioTsumaCigv = 0, costoTsumaCigv = 0, difTsumaCigv = 0;
		double pesoTotal = 0;
		int canti = 0;
		for (int i = 0; i < modelo2.getRowCount(); i++) {
			if (modelo2.getValueAt(i, 11).equals("ERROR 0")
					|| modelo2.getValueAt(i, 11).equals("ERROR 1")
					|| modelo2.getValueAt(i, 11).equals("ERROR 2")
					|| modelo2.getValueAt(i, 11).equals("ERROR 2.2")
					|| modelo2.getValueAt(i, 11).equals("UND")) {

			} else {
				canti = Integer.parseInt("" + modelo2.getValueAt(i, 1));
				precioTsuma = precioTsuma
						+ Double.parseDouble("" + modelo2.getValueAt(i, 6));
				costoTsuma = costoTsuma
						+ Double.parseDouble("" + modelo2.getValueAt(i, 8));
				difTsuma = difTsuma
						+ Double.parseDouble("" + modelo2.getValueAt(i, 9));
	...
	public int guardar() {
		...
		AccesoBD objAccesoBD = new AccesoBD();
		objAccesoBD.crearConexion();
		// NOTA :AL GUARDAR SE TIENE QUE DAR CUENTA QUE NO GUARDE CUANDO HAY
		// ERROR
		int tam = arrayExcel.tamaño();
			...
			for (int i = 0; i < arrayExcel.tamaño(); i++) {
				...
				if (modelo2.getValueAt(i, 11).equals("ERROR 0")
					...
					if (objExcel.getCod_cli() == 0) {
						...
						numi = numi + 1;
						// cod_cot, num_item, cod_prove, cod_prod, cod_mar,
						// cod_umed, cod_cli,
						// cod_prodalt, cod_cam, can_cot, porga_cot, fec_cot,
						// cod_ven, ide_cot
						/*
						 * cod_cot, num_item, cod_prove, cod_prod, cod_mar,
						 * cod_umed, cod_cli, cod_cos, cod_prodalt, cod_cam,
						 * ref_cot, can_cot, porga_cot, fec_cot, cod_ven,
						 * esten_cot, estoc_cot, ide_cot
						 */
						guardarCotizacion = "INSERT INTO tb_cotizacion(cod_cot, num_item, cod_prove, cod_prod, cod_mar, cod_umed, cod_cli, "
								+ " cod_cam, ref_cot, can_cot, porga_cot, fec_cot, cod_ven, esten_cot, estoc_cot, ide_cot,tiempo_entrega,est_emp,pesoCarr,est_envio) VALUES('"
								+ cod_cot
								+ "','"
								+ numi
								+ "','"
								+ objVarios.getCodprove()
								+ "','"
								+ objVarios.getCodprod()
								+ "','"
								+ objVarios.getCodmar()
								+ "','"
								+ objVarios.getCod_umed()
								+ "','"
								+ objExcel.getCod_cli()
								+ "','"
								+ codigoFecha()
								...
								+ 0
								+ "','"
								+ modelo2.getValueAt(i, 14)
								+ "','" + estado + "','0','0');";
						System.out.println(guardarCotizacion);
						try {
							op = objAccesoBD
									.ejecutarActualizacion(guardarCotizacion);
						} catch (Exception e) {
			...
			for (int i = 0; i < objSCotInc.tamaño(); i++) {
				...
				if (modelo2.getValueAt(i, 11).equals("ERROR 0")
					...
					if (objExcel.getCod_cli().equals("0")) {
						...
						numi = numi + 1;
						System.out.println("Estos son los codigos que salen: "
								+ objExcel.getCod_prove() + " /"
								+ objExcel.getCod_prod() + " /"
								+ objExcel.getCod_mar() + " /"
								+ objExcel.getCod_umed() + "/"
								+ objExcel.getCod_prod());
			...
			for (int i = 0; i < arrayExcel.tamaño(); i++) {
				...
				if (objExcel.getCod_cli() == 0) {
					...
					String insertarPregunta = "INSERT INTO tb_cotizacionINCONCLUSA(ide_cot_inc,num_item,cod_ven,cod_cli,fecha_coti, cantidad,"
							...
							+ " precio_total, precio_costo, costo_total, diferencia, fecha, porcentaje, nom_prove,cod_prove,cod_prod,cod_mar,cod_umed,peso_prod,tiempo_entrega,est_emp,costo) VALUES('"
							+ ide_cot_inc
							+ "','"
							+ numi
							+ "','"
							...
							+ "','"
							+ modelo2.getValueAt(i, 4)
							+ "','"
							+ modelo2.getValueAt(i, 5)
							+ "','"
							+ modelo2.getValueAt(i, 6)
							+ "','"
							+ modelo2.getValueAt(i, 7)
							+ "','"
							+ modelo2.getValueAt(i, 8)
							+ "','"
							+ modelo2.getValueAt(i, 9)
							+ "','"
							+ modelo2.getValueAt(i, 10)
							+ "','"
							+ modelo2.getValueAt(i, 11)
							+ "','"
							+ modelo2.getValueAt(i, 12)
							+ "','"
							+ objServletVarios.obtener(i).getCodprove()
							+ "','"
							+ objServletVarios.obtener(i).getCodprod()
							+ "','"
							+ objServletVarios.obtener(i).getCodmar()
							+ "','"
							+ objServletVarios.obtener(i).getCod_umed()
							+ "','"
							+ modelo2.getValueAt(i, 13)
							+ "','"
							+ modelo2.getValueAt(i, 14)
							+ "','"
							+ estado
							+ "','" + objVarios.getCostoreal() + "');";
					System.out.println(insertarPregunta);

					try {
						op = objAccesoBD
								.ejecutarActualizacion(insertarPregunta);
					} catch (Exception e) {
						// TODO: handle exception
						op = 0;
		...
		if (objServletVarios.tamaño() != 0) {
			for (int i = 0; i < arrayExcel.tamaño(); i++) {
				...
				if (modelo2.getValueAt(i, 11).equals("ERROR 0")
					...
					item = item + 1;
					arregloCadenas[0] = objExcel.getSeñor().toString()
							.toUpperCase();
					arregloCadenas[1] = objExcel.getAtencion().toString()
							.toUpperCase();
					arregloCadenas[2] = objExcel.getDireccion().toString()
							.toUpperCase();
					...
					arregloCadenas[3] = "TEL:" + objExcel.getTelefono()
							+ rpmnextel + " CEL:" + objExcel.getCel()
							+ " E-MAIL:" + objExcel.getMail();
					arregloCadenas[4] = objGlobal.COD_VEN;
					arregloCadenas[5] = objExcel.getTipocliente().toString()
							.toUpperCase();
					arregloCadenas[6] = objExcel.getCod_cli();
					arregloCadenas[7] = "COTIZACION Nº" + cod;
					arregloCadenas[8] = objFecha.fechaActual();
					arregloCadenas[9] = Double.parseDouble(""
							+ tipoCambioVenta());
					arregloCadenas[10] = ref.toString().toUpperCase();
					for (int j = 0; j < 10; j++) {
						if (j == 0) {
							arregloObjetos[item][j] = Integer.parseInt(""
									+ objExcel.getCant());
						} else if (j == 1) {
							arregloObjetos[item][j] = objVarios.getUmed()
									.toString();
						} else if (j == 2) {
							arregloObjetos[item][j] = objVarios.getProducto()
									.toString().toUpperCase();
						} else if (j == 3) {
							arregloObjetos[item][j] = objVarios.getMarca()
									.toString().toUpperCase();
						} else if (j == 4) {
							arregloObjetos[item][j] = Double.parseDouble(""
									+ modelo2.getValueAt(i, 5));
						} else if (j == 5) {
							arregloObjetos[item][j] = Double.parseDouble(""
									+ modelo2.getValueAt(i, 7));
						} else if (j == 6) {
							arregloObjetos[item][j] = modelo2.getValueAt(i, 10)
									.toString();
						} else if (j == 7) {
							arregloObjetos[item][j] = modelo2.getValueAt(i, 12)
									.toString();
						} else if (j == 8) {
							arregloObjetos[item][j] = ""
									+ objVarios.getCodprove();
						} else {
							arregloObjetos[item][j] = modelo2.getValueAt(i, 13)
									.toString();
						}
			...
			for (int i = 0; i < objSCotInc.tamaño(); i++) {
				...
				if (modelo2.getValueAt(i, 11).equals("ERROR 0")
					...
					item = item + 1;
					arregloCadenas[0] = objclinc.getSeñor().toString()
							.toUpperCase();
					arregloCadenas[1] = objclinc.getAtencion().toString()
							.toUpperCase();
					arregloCadenas[2] = objclinc.getDireccion().toString()
							.toUpperCase();
					String rpm = objclinc.getRpm(), nex = objclinc.getNextel(), rpmnextel = objclinc
							.getNextel();
					if (rpm.equals("0") || rpm.equals("") || rpm.equals(" ")) {
						rpmnextel = " NEX:" + nex;
					} else {
					...
					arregloCadenas[3] = "TEL:" + objclinc.getTelefono()
							+ rpmnextel + " CEL:" + objclinc.getCel()
							+ " E-MAIL:" + objclinc.getMail();
					arregloCadenas[4] = objGlobal.COD_VEN;
					arregloCadenas[5] = objclinc.getTipocliente().toString()
							.toUpperCase();
					arregloCadenas[6] = objExcel.getCod_cli();
					arregloCadenas[7] = "COTIZACION Nº" + cod;
					arregloCadenas[8] = objFecha.fechaActual();
					...
					for (int j = 0; j < 10; j++) {
						if (j == 0) {
							...
							arregloObjetos[item][j] = objExcel.getUnimedida()
									.toString();
						} else if (j == 2) {
							arregloObjetos[item][j] = objExcel.getDesc_prod()
									.toString().toUpperCase();
						} else if (j == 3) {
							arregloObjetos[item][j] = objExcel.getMarca()
									.toString().toUpperCase();
						} else if (j == 4) {
							arregloObjetos[item][j] = Double.parseDouble(""
									+ modelo2.getValueAt(i, 5));
	...
	public void eliminarCotInc() {
		...
		/*
		 * (COD_CLI, COD_VEN, FEC_REP, FECC_REP, TIP_REP, CAL_REP, OBS_REP,
		 * EST_REP)
		 */
		String sql = "DELETE FROM tb_cotizacioninconclusa "
				+ "WHERE IDE_COT_INC='" + ide_cot_inc + "' AND COD_VEN='"
				+ cod_ven + "';";
		...
		for (int i = 0; i < objSGuarProd.tamaño(); i++) {
			...
			if (objCotVarios.getCod_rubro().equals("0")) {
				...
				String sql = "INSERT INTO tb_prueba(cod_prueba,cod_rubro,nom_rubro,nom_prod,uni_prod,cant_prod,est_prueba) "
						...
						+ "','"
						+ objCotVarios.getNom_prod()
						+ "','"
						+ objCotVarios.getUnidad()
						+ "','"
						+ objCotVarios.getCantidad() + "','ACTIVADO');";
				System.out.println(sql);
				...
				if (op == 0) {
					...
					System.out
							.println("Se Ingreso los prod no existentes Correctamente ");
	...
	public void listarProdNoEncontrados() {
		...
		try {
			...
			while (rs.next()) {
				BeanGuardarProdNoEncontrados objR = new BeanGuardarProdNoEncontrados(
						rs.getString(1), rs.getString(2), rs.getString(3),
	...
	public void listarProdNoEncontradosPorRubro(String cad) {
		...
		try {
			...
			while (rs.next()) {
				conta1 = conta1 + 1;
				BeanGuardarProdNoEncontrados1 objR = new BeanGuardarProdNoEncontrados1(
						rs.getString(1), rs.getString(2), rs.getString(3),
						rs.getString(4), rs.getString(5));
	...
	public int contarProdNoEncontrados() {
		...
		String sql = "select cod_rubro,nom_rubro,nom_prod,uni_prod,cant_prod from tb_prueba"
				+ " where est_prueba='ACTIVADO' " + " group by cod_rubro;";
		...
		if (tam > 0) {
			for (int i = 0; i < objSGuarProd1.tamaño(); i++) {
				...
				for (int j = 0; j < 4; j++) {
					...
					if (j == 0) {
						arregloObjetos2[item][j] = objRep.getCantidad()
								.toUpperCase();
					} else if (j == 1) {
						arregloObjetos2[item][j] = objRep.getUnidad()
								.toUpperCase();
					} else if (j == 2) {
						arregloObjetos2[item][j] = objRep.getNom_prod()
								.toUpperCase();
					} else {
						arregloObjetos2[item][j] = objRep.getNom_rubro()
								.toUpperCase();
					}
				}
			}
	...
	public void listarProveedoresCorreo(String cod) {
		...
		String sql = "SELECT  PROVE.COD_PROVE,PRU.cod_rubro,PROVE.PER_PROVE, PROVE.SEX_PROVE,PROVE.MAIL_PROVE "
				...
				+ "WHERE PRU.est_prueba='ACTIVADO' AND pru.cod_rubro='"
				+ cod
		...
		try {
			...
			while (rs.next()) {
				...
				BeanProveCorreo objPC = new BeanProveCorreo(rs.getString(1),
						rs.getString(2), rs.getString(3), rs.getString(4),
						rs.getString(5));
	...
	public void ingresarMailsAutoProve(String cod_prov) {
		...
		if (op == 0) {
			...
			System.out.println("Se ingreso Correctamente los MailsProvee "
					+ "¡¡¡¡¡¡¡¡ GRACIAS TOTALES !!!!!!!");
		}
		objAccesoBD.cerrarConexion();
	...
	/**********************************************************************/
	public void ejecutarbusquedaporparametro(String data_parametro) {
		abrirArchivo();
		reescribir();
		buscarDetalles();
		calcular();
	...
	public void tiempoEntrega() {
		...
		objCamTiemEntrega.txtCantidad
				.setText("" + modelo2.getValueAt(fila, 14));
		String[] botones = { "Aceptar", "Cancelar" };// Esto es el nombre
		int showOptionDialog = JOptionPane.showOptionDialog(this,
				objCamTiemEntrega, "Cambiar Tiempo Entrega", 0, -1, null,
				botones, "Cerrar"
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == btnBuscarCliente) {
			...
			int showOptionDialog = JOptionPane.showOptionDialog(this,
					objBuscarCliAut, "Buscar Cliente", 0, -1, null, botones,
					"Cerrar"
			...
			try {
				...
				if (tam > 0) {
					for (int i = 0; i < arrayExcel.tamaño(); i++) {
						...
						BeanExcel objExcel = arrayExcel.obtener(i);
						objExcel.setCod_cli(Integer
								.parseInt(objBuscarCliAut.codcli));
			...
			if (txtReferencia.getText().equals("")) {
				...
				if (objServletVarios.tamaño() != 0) {
					...
					try {
						...
						objPasarExcel
								.crearExcel(arregloCadenas, arregloObjetos);
					...
					System.out
							.println("Si entra a guardar pero hay null pointer exception   "
									+ cod);
					arregloCadenas = new Object[11];
		...
		if (e.getSource() == btnCargar) {
			...
			if (objServletVarios.tamaño() == 0) {
				...
				String[] botones = { "Aceptar" };// Esto es el nombre
				int showOptionDialog = JOptionPane.showOptionDialog(this,
						objCargarCotInc, "Cargar Cotizacion Inconclusa", 0, -1,
						null, botones, "Cerrar"
		...
		if (e.getSource() == btngprod) {
			...
			TranCotizacionAutomatica objtrans = new TranCotizacionAutomatica();
			...
			if (tamañoProdExcel() != 0) {
				...
				String numCoti = "" + retornaUltimoCodProdInexistente2(), cod_rubro = "";
				int cont = 0, cantLetras = 0, cantLetras1 = 0;

				for (int i = 0; i < contarProdNoEncontrados(); i++) {
					try {

						cont++;
						BeanGuardarProdNoEncontrados objProd = objSGuarProd
								.obtener(i + conta1);
						...
						if (cont == 1) {
							...
							if (cantLetras == cantLetras1) {
								conta1 = conta1 - 1;
								listarProdNoEncontradosPorRubro(objProd
										.getNom_rubro());
								...
								objProdExcel.crearExcel(arregloCadenas1,
										arregloObjetos2, numCoti,
										palabrasSinespacio(objProd
												.getNom_rubro()));
								rutaGlobal = objProdExcel.fileGlobal;
								valor = true;

								for (int j = 0; j < tamPC; j++) {
									BeanProveCorreo objPc = objSProvCorreo
											.obtener(j);
									...
									objMailProv.envio1();
									System.out.println("CORREO PROVE: "
											+ objPc.getPer_prove() + "/ "
											+ objPc.getMail_prove());
								}
							}
							...
							if (cantLetras == cantLetras1) {
								...
								cantLetras = cantPalabras(objProd
										.getNom_rubro());
								listarProdNoEncontradosPorRubro(objProd
										.getNom_rubro());
								listarProveedoresCorreo(objProd.getCod_rubro());
			...
			try {
				...
				if (tam > 0) {
					...
					for (int i = 0; i < objCotizacionBorrar.tamaño(); i++) {
						...
						BeanCoti_borrar objcotborrar = objCotizacionBorrar
								.obtener(i);

						Object[] array = { Integer.parseInt("" + contM),
								objcotborrar.getCant(), objcotborrar.getUnd(),
								objcotborrar.getDesc(),
								objcotborrar.getMarca(),
								objcotborrar.getPunit(),
								objcotborrar.getPtotal(),
								objcotborrar.getPcosto(),
								objcotborrar.getCtotal(),
								objcotborrar.getDif(), objcotborrar.getFecha(),
								objcotborrar.getPorc(), objcotborrar.getProv(),
								objcotborrar.getPestotal(),
								objcotborrar.getTiempo_entrega() };
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getSource() == tblLista & tblLista.getSelectedColumn() == 1) {
			...
			if (objServletVarios.tamaño() != 0) {
				...
				if (Punit.equals("ERROR 0") || Punit.equals("ERROR 1")
					...
					objCambiarCant.txtCantidad.setText(""
							+ modelo2.getValueAt(fila, 1));
					String[] botones = { "Aceptar", "Cancelar" };// Esto es el
																	// nombre
					int showOptionDialog = JOptionPane.showOptionDialog(this,
							objCambiarCant, "Cambiar Cantidad", 0, -1, null,
							botones, "Cerrar"
				...
				if (Punit.equals("ERROR 0") || Punit.equals("ERROR 1")
					...
					if (showOptionDialog == 1 || showOptionDialog == -1) {
						...
						// BeanExcel objExcel=arrayExcel.obtener(fila);
						BeanCotizacionInconclusa objCotInc = objSCotInc
								.obtener(fila);
						System.out.println("ES LA FILA :" + fila);
		...
		if (e.getSource() == tblLista & tblLista.getSelectedColumn() == 3) {
			...
			/*
			 * if(estGTemp==0){ ide_cot=retornaUltimoCodCotizacionInconclusa();
			 * } else{ ide_cot=retornaUltimoCodCotizacionInconclusa2(); }
			 */
			...
			BuscarProductoCotizacion objBuscarProdCot = new BuscarProductoCotizacion(
					objMenuP);

			objBuscarProdCot.setVisible(true);
			objBuscarProdCot.pack(); // para darle tamaño automático a la
										// ventana.

			if (objBuscarProdCot.COD_PROD == null) {
				// ACA SE GUARDARAN LOS PRODUCTOS QUE NO EXISTAN EN LA BD PARA
				// LUEGO ENVIARLOS
				...
				try {
					...
					System.out.println("ESTO ES LO QUE SE OBTIENE :"
							+ cod_rubro + " / " + nom_rubro + " / " + nom_prod
							+ " / " + und + " / " + canti);
					verificar = 1;
				...
				System.out.println("ESTE ES EL PRIMER CODIGO DEL PRODUCTO:"
						+ COD_PROD);
				...
				System.out.println("ESTO ES LO QUE VIENE DESPUES DE BUSCAR 1:"
						+ COD_PROD + "," + NOM_PROD + "," + COSTE + "," + MARCA
						+ "," + UMED + "," + FECHA + "," + COD_PROVEE + ","
				...
				if (objServletVarios.tamaño() != 0) {
					...
					System.out.println("ESTOS SON DIFERENTES:" + NOM_PROD
							+ "||" + NOM_PRODALT);
					...
					if (tiempoEntrega.equals("ERROR 0")
							|| tiempoEntrega.equals("ERROR 1")
							|| tiempoEntrega.equals("ERROR 2")
							|| tiempoEntrega.equals("ERROR 2.2")
							|| tiempoEntrega.equals("UND")) {
						modelo2.setValueAt("1 Dia", fila, 14);// Tiempo Entrega
						tiempoEntrega();
					}
					calcularParte(fila);// PARA QUE SE CALCULE Y SE MUESTRE EN
										// LA TABLA
					...
					if (estGTemp == 0) {
						...
						objAccesoBD.crearConexion();
						/*
						 * ide_cot_inc,num_item,cod_ven,cod_cli,fecha_coti,cantidad
						 * ,unimedida,desc_prod,marca,precio_unit,precio_total,
						 * precio_costo
						 * ,costo_total,diferencia,fecha,prorcentaje,nom_prove;
						 */
						...
						String sql = "update tb_cotizacioninconclusa set "
								...
								+ modelo2.getValueAt(fila, 1)
								+ "',unimedida='"
								+ modelo2.getValueAt(fila, 2)
								+ "', "
								+ "desc_prod='"
								+ modelo2.getValueAt(fila, 3)
								+ "',marca='"
								+ modelo2.getValueAt(fila, 4)
								+ "', "
								+ " precio_unit='"
								+ modelo2.getValueAt(fila, 5)
								+ "',precio_total='"
								+ modelo2.getValueAt(fila, 6)
								+ "',precio_costo='"
								+ modelo2.getValueAt(fila, 7)
								+ "', "
								+ " costo_total='"
								+ modelo2.getValueAt(fila, 8)
								+ "',diferencia='"
								+ modelo2.getValueAt(fila, 9)
								+ "',fecha='"
								+ modelo2.getValueAt(fila, 10)
								+ "',"
								+ " porcentaje='"
								+ modelo2.getValueAt(fila, 11)
								+ "',nom_prove='"
								+ modelo2.getValueAt(fila, 12)
								+ "',cod_prove='"
								+ COD_PROVEE
								+ "' ,"
								+ " cod_prod='"
								...
								+ "',"
								+ " peso_prod='"
								+ modelo2.getValueAt(fila, 13)
								+ "',tiempo_entrega='"
								+ modelo2.getValueAt(fila, 14)
								+ "',COSTO='"
								+ formato(Double.parseDouble(COSTE))
								+ "' "
								+ " WHERE num_item='"
								+ modelo2.getValueAt(fila, 0)
								+ "' and FECHA_COTI='"
								+ fech
								+ "' and COD_VEN='"
								+ cod_ven
								+ "' and IDE_COT_INC = '" + ide_cot + "';";
		...
		if (e.getSource() == tblLista
				& (tblLista.getSelectedColumn() == 5
						|| tblLista.getSelectedColumn() == 6
						|| tblLista.getSelectedColumn() == 7
						|| tblLista.getSelectedColumn() == 8 || tblLista
						.getSelectedColumn() == 9)) {
			...
			if (objServletVarios.tamaño() != 0) {
				System.out
						.println("ESTE ES LA FILA QUE ESTA AGARRANDO:" + fila);
				BeanExcel objExcel = arrayExcel.obtener(fila);
				BeanVarios objVarios = objServletVarios.obtener(fila);
				...
				objescCal.txtCostoTotal.setText(""
						+ modelo2.getValueAt(fila, 7));
				objescCal.txtTotalCosto.setText(""
						+ modelo2.getValueAt(fila, 8));
				objescCal.txtDiferencia.setText(""
						+ modelo2.getValueAt(fila, 9));
				objescCal.txtFactor.setText("" + modelo2.getValueAt(fila, 11));
			} else {
				// int fila = tblLista.getSelectedRow();
				System.out.println("ESTE ES LA FILA QUE ESTA AGARRANDOSSSSS:"
						+ fila);
				BeanCotizacionInconclusa objCoti = objSCotInc.obtener(fila);
				// BeanVarios objVarios=objServletVarios.obtener(fila);
			...
			String[] botones = { "Aceptar", "Cancelar" };// Esto es el nombre
			int showOptionDialog = JOptionPane.showOptionDialog(this,
					objescCal, "Calcular Producto", 0, -1, null, botones,
					"Cerrar"

			);
			if (showOptionDialog == 1 || showOptionDialog == -1) {// CANCELAR Y
																	// X
				System.out.println("NO COGIO NADA");
			} else {// ACEPTAR
				objGUI.mostrarAviso("este es el valor:"
						+ objescCal.txtFactor.getText());
		...
		if (e.getSource() == tblLista & tblLista.getSelectedColumn() == 14) {
			...
			if (tiempoEntrega.equals("ERROR 0")
				...
				objCamTiemEntrega = null;
				objCamTiemEntrega = new CambiarTiempoEntrega();
				objCamTiemEntrega.txtCantidad.setText(""
						+ modelo2.getValueAt(fila, 14));
				String[] botones = { "Aceptar", "Cancelar" };// Esto es el
																// nombre
				int showOptionDialog = JOptionPane.showOptionDialog(this,
						objCamTiemEntrega, "Cambiar Tiempo Entrega", 0, -1,
						null, botones, "Cerrar"
