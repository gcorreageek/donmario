...
*/
public class BuscarCotizacionTarea extends JPanel implements ActionListener, KeyListener, MouseListener {
	...
	boolean valor=true;
	public static int selecciona=0,est=0;
	...
	public void listarCot(){
     ...
     public void listarCotTarea(){
 		
 		objBCM.eliminarTodo();
 		objRep=new Reporte();
 		String cod=objRep.codigocliente;
 		int n=modelo2.getRowCount();
 		for (int fila=0; fila<n; fila++)
 		modelo2.removeRow(0);
 		AccesoBD objAccesoBD = new AccesoBD();
 		...
 		String sql=" SELECT COT.COD_COT , CLI.NOM_CLI ,CLI.CONA_CLI," +
 				...
 				" INNER JOIN tb_vendedores VEN " +
 				"  ON VEN.COD_VEN=COT.COD_VEN " +
 				" where COT.est_emp=1 and CLI.COD_CLI ='"+cod+"' " +
 				" GROUP BY COT.COD_COT , COT.IDE_COT order by cot.cod_cot desc ,cot.ide_cot desc";
 		//System.out.println();
 		ResultSet rs = objAccesoBD.ejecutarConsulta(sql);
 		try {
